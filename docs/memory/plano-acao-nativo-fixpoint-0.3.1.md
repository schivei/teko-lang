...
- **B2. Remover REPL + remanescente de VM** (mantendo **teko doc** e **teko lint**, que apoiam o LSP):
  ...
  qualquer código remanescente de VM. NÃO por revert à mão — pelo redo do dreno na ordem certa.
...
- **C1.** `fix/union` linear, sem REPL/VM, com journal reconciliado e o fix nativo → PR #107
...
2. **B (journal + REPL/VM)** em paralelo — agente aplica a solução do journal e refaz o dreno sem
   REPL/VM.
...
- REPL e remanescente de VM FORA; teko doc e teko lint FICAM.
