...
ordinary data and is compared as data — no union, no claim. **If the owner intended `uptr | null` as
a required spelling, this is the measurement that must reach him**, because it needs a niche arm for
`Uptr` that would be a *fiction* (an integer has no unused encoding), not a small addition.
