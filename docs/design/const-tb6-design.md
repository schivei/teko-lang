...
completa e no seed: `LDataReloc`/`LRodata.relocs` na LIR (`lir.tks:154/175`); o interpretador resolve
...
3. **PRÉ-REQUISITO DE LEITURA (constatação crítica — ponto 1/3):** a lowering
   COMPARTILHADA (interpretador + nativo) **NÃO lê hoje um campo slice de struct** — `lower_fat_expr`
...
Isto é coerente com o fato de que o caminho nativo/interpretador **nunca materializa** um `AbiDescriptor`
hoje (honest-stopa antes). O layout de 8 bytes é um estado morto — nenhum teste nativo/interpretador
...
lowering compartilhada → **nem o interpretador nem o nativo** as compilam hoje. Os testes ABI/regalloc
...
agregados — plano §5.2 tabela). O interpretador consome a MESMA LIR de `lower.tks`, então
herda o mesmo honest-stop.

**Consequência para T-B6:** o espelho SOURCE-level dual-engine (um `const AAPCS64` lido
IDÊNTICO no interpretador e no nativo — o objetivo declarado de T-B6, T-B5 §5.8) exige que a lowering
...
- interpretador: `resolve_rodata_relocs` sobrescreve cada slot de ponteiro
...
- **Slot de ponteiro = 8 bytes zero + `LDataReloc(fieldoff, leaf_sym)`.** Resolvido pelo interpretador
...
- **Array-alvo = entrada rodata SEPARADA**, símbolo próprio, `relocs` vazio (é flat-scalar
  ...
  target` é um NOME DE SÍMBOLO (`lir.tks:154`), e a resolução (interpretador/writers) é por símbolo —
...
segurança:
- Nenhum struct do corpus com campo fat é materializado hoje no caminho nativo/interpretador (§1.4),
  então nenhum teste nativo/interpretador que passa depende do layout de 8 bytes → a ampliação não
  regride nada.
- O C backend usa o layout C próprio (`tk_slice_<elem>` já é `{ptr,len}` = 16B) — inalterado.
- A ampliação ALINHA o layout nativo/interpretador com o C (a direção correta) e é o que o serializer
...
- **Leitura via LFieldAddr/LLoad nas duas engines:** o reader (§4) baixa `abi.gpr_arg` para
  ...
  (I64). No interpretador (T-B5 §1.2) um `LLoad` lê UMA célula: o ptr vem da célula `base+fieldoff`
 ...
 * mesmos símbolos → `LDataReloc.target`, as bases cumulativas do interpretador e os offsets re-baseados
 ...
 * campo declarada), então as bases cumulativas (interpretador) e os offsets re-baseados (writers) são
...
rodata-load no nativo/interpretador, ou inline no C backend) — os bytes que regalloc EMITE não mudam.
...
- **interpretador goldens (o twin de testes do interpretador):** os testes existentes (relocs vazio) ficam
...
### 7.3 (reader, o twin de testes do interpretador) ler um campo slice de struct const — deref end-to-end
...
**exit 10** no interpretador. (`K.xs[1]` → 20; `K.xs.len` → 2.) Prova que T-B5 resolve o slot e o reader
...
- **O fonte T-B6 compila no seed T25?** SIM. T25 (0.3.0.25) tem a capability data→data
  CONSUMIDORA (interpretador/writers/encoder-bridge/wasm) no seed. O fonte de T-B6 (o `const AAPCS64` +
  ...
  reader nativo/interpretador que T-B6 adiciona é exercitado pelas fixtures dual-engine (§7.3/§7.5), não
  pelo bootstrap.
...
- **Ambas as engines:** as fixtures §7.3/§7.5 rodam interpretador + nativo pelo harness `.tkt` — a prova
...
| E3 | `src/lir/lower.tks` | `lower_fat_field` + braço `TFieldAccess` em `lower_fat_expr`; (opc.) `store_struct_fields` grava as 2 metades (§4.2/§4.3) | o twin de testes do interpretador §7.3 |
...
- **RITUAL POINT — fim de T-B6:** gate COMPLETO — todos os goldens interpretador (o twin de testes do interpretador)
  ...
  **fixpoint gen1==gen2** + ambas as engines (interpretador + nativo) nas fixtures dual-engine + 100% de
...
3. **O reader não existe na lowering compartilhada (a constatação crítica, §1.4).** Sem ele o
   ...
   de struct first-class no nativo/interpretador (build via §4.3 + read via §4.2) — um ganho geral que
...
4. **Ampliar o campo fat p/ 16 bytes muda o layout de TODO struct com campo fat.** *Resolução
   (não é regressão):* nenhum struct-fat é materializado hoje no caminho nativo/interpretador (honest-stopa,
   §1.3/§1.4), então nenhum golden nativo/interpretador que passa depende dos 8 bytes; o C backend usa
   layout C próprio (já 16B). A ampliação alinha nativo/interpretador com o C. Asserido em §7.4; fixpoint
   confirma.

5. **Tolerância multi-byte do interpretador no slot de `len`.** O interpretador semeia byte-por-célula e um `LLoad`
...
- doc-comments em `encode_x86_64.tks`, `isel_x86_64.tks`, `regalloc_x86.tks`,
  um regalloc.
- fixtures: `lower_test.tkt`, o twin de testes do interpretador, `abi_*_test.tkt`, `regalloc_*_test.tkt`,
...
- **Sem tocar:** os C twins; `serialize_const:5142` braço `_` (mantido p/ formas
  ...
  Mach-O/COFF/wasm/ELF e o interpretador já resolvem).
</content>
</invoke>
