# T-B5 — interpretador resolve o ponteiro rodata-INTERNO + o encoder nativo fecha a cadeia data→data (#594 Tier-B)
...
> Escopo de T-B5 (verbatim §8): "interpretador (o interpretador): resolve a rodata-INTERNAL
...
> ensina o interpretador a materializar o endereço-alvo de um ponteiro rodata→rodata, E — porque
...
> do interpretador e os objetos nativos são **byte-idênticos**, e todos os goldens/fixpoint ficam
> intactos.

> **Correção de referência de linha.** O plano §8 aponta o interpretador, que hoje
...
> imagem rodata do interpretador é construída — espelhando o emit-time do T-B4 (`wasm_relocate_
...
### 1.1 A representação de rodata no interpretador — memória linear própria, célula-por-byte, endereço = índice de célula

O interpretador tem a sua PRÓPRIA memória linear, `MemStore` (o interpretador): uma lista
...
para o alvo). Nada hoje protege esse caminho no interpretador (`encode_rodata`/`honest_data_reloc`
é do caminho NATIVO; o interpretador consome `LModule` direto, como o wasm). T-B5 preenche esse
...
1. **Espelha T-B4 e os writers nativos — "um lugar só, zero custo por load".** T-B4
   ...
   emissão. O interpretador já tem o análogo perfeito: `seed_rodata` constrói a imagem rodata UMA
   vez antes de a entry rodar. Resolver ali é o mesmo idioma "recompute-don't-thread"
   que o interpretador já usa (`rodata_base_of` re-deriva a base a cada `LGlobalAddr`; aqui
...
### 2.2 Como o interpretador SABE que um slot é ponteiro interno — lê `LRodata.relocs` (não infere)

O interpretador **não infere** nada dos bytes: lê `m.rodata[i].relocs` (a lista
...
consome `LRodata.relocs` DIRETAMENTE (nível LIR)"). O interpretador, como o wasm, não usa
...
devolve e um deref posterior usa como base. No interpretador um endereço É um índice de célula
...
- Para a entrada `i` com base `B_i` (= `rodata_base_of(entry_i.symbol).addr`, a soma
  ...
  `[B_i + offset, B_i + offset + 8)` (8 bytes zero). O interpretador **sobrescreve a célula
...
- As 7 células altas do slot (`B_i + offset + 1 .. +7`) permanecem 0 — o ponteiro é lido
  ...
  de 8 bytes do slot (paridade nativa/wasm) é irrelevante para o interpretador: o interpretador usa a célula
...
Colocadas em o twin Teko do interpretador logo após `store_bytes` (`:197`) e antes de `RodataHit`
 ...
 * célula (o "endereço" na memória do interpretador) da primeira célula do datum-alvo — na célula
 ...
 * baixa recebe o endereço (o interpretador lê o ponteiro por um load de UMA célula), as altas
 * permanecem zero.
 *
 ...
 * `resolve_entry_relocs`. É o análogo, no interpretador, do `wasm_relocate_rodata` (T-B4) e das
...
## 3. Consistência interpretador≡native (Lei D4 / dual-engine) — o valor resolvido é o análogo semântico do endereço absoluto

O mesmo programa Tier-B tem de se comportar idêntico no interpretador e no nativo/wasm. Prova de
que o índice de célula que o interpretador grava é o análogo semântico do endereço absoluto:
...
1. **O que cada engine grava no slot.**
   ...
   - interpretador (T-B5): `rodata_base_of(target).addr` = `soma_cumulativa_até_o_alvo` = a célula-base do alvo na memória do interpretador.
   - **Os três são a MESMA quantidade semântica:** "o endereço, no espaço de endereços daquela engine, da primeira unidade do datum-alvo." O interpretador tem base 0 e passo 1-célula-por-byte; o nativo tem `section_base` e passo 1-byte; o wasm tem `wasm_rodata_base()`. A base e a escala diferem, mas o REFERENTE é idêntico.
...
T-B5, junto da resolução do interpretador.** Não é opcional — é EXIGIDO pela regra de bootstrap,
...
1. **A regra de bootstrap-seed obriga.** "O corpus não pode USAR uma feature que ainda
   ...
   seed NO bump. O bump é depois de T-B5. Os writers (T-B2/T-B3), o wasm (T-B4) e o interpretador
...
### 5.1 (interpretador) resolve grava o endereço do alvo — deref end-to-end lê o byte-alvo

o twin de testes do interpretador, construindo `LModule` à mão. Helper novo `iwt_rodata_rel`
 ...
 * dos testes de resolução do interpretador (#594 T-B5).
 *
...
### 5.2 (interpretador) unitário `resolve_rodata_relocs` — a célula do slot recebe o índice do alvo
...
### 5.3 (interpretador) base cumulativa não-trivial — o alvo depois de um datum de padding
...
### 5.4 (interpretador) colapso byte-idêntico quando `relocs` vazio
...
### 5.5 (interpretador) alvo desconhecido → run falha com o sentinela negativo (nunca miscompile silencioso)
...
### 5.6 (interpretador) `rodata_base_of` inalterado
...
`.tkt`, então **rodam idênticas no interpretador e no harness nativo** (mesma saída de `error`/exit)
— isso já satisfaz o requisito dual-engine ao nível LIR/encoder. O espelho SOURCE-level
verdadeiro (um `const AAPCS64: AbiDescriptor = …` lido no interpretador E no nativo produzindo o
...
|---|---|---|---|
| E1 | o twin Teko do interpretador | `resolve_entry_relocs` + `resolve_rodata_relocs` (§2.4), após `store_bytes` (`:197`) | o twin de testes do interpretador compila |
| E2 | o twin Teko do interpretador | `seed_rodata -> MemStore \| error` + passo de resolução (§2.4); `interp_lmodule` mapeia erro→`-1` (`:160`) | o twin de testes do interpretador (F13/F14 e todos os goldens do interpretador byte-idênticos, relocs vazio) |
| E3 | o twin de testes do interpretador | helper `iwt_rodata_rel` + `iwt_tb5_deref_module` + fixtures §5.1–§5.6 | os próprios testes novos (verde) |
...
- **RITUAL POINT — fim de T-B5:** gate COMPLETO — todos os goldens do interpretador
  (o twin de testes do interpretador) e de backend (`encode_*_test.tkt`, `objfile_*_test.tkt`,
  `lower_test.tkt`, `tkb_test.tkt`) byte-idênticos + **fixpoint gen1==gen2** + ambas as
  engines (interpretador + nativo) + 100% de cobertura do delta (§5). Zero bytes mudam (produtor
...
- **🔑 SEED BUMP #3 (0.3.0.25) — DEPOIS do gate de T-B5.** A capability data→data está
  completa no seed (encoders nativos + writers ELF/Mach-O/COFF + wasm + interpretador). Isto
...
3. **O modelo célula-por-byte vs a largura de 8 bytes do slot.** Um erro aqui gravaria
   o endereço na célula errada. *Resolução:* o interpretador lê o ponteiro por UM `LLoad` de UMA
...
4. **Abrir `encode_rodata` amplia o crumb além do interpretador.** *Soft tension, não HALT.* É
...
6. **Análogo-semântico interpretador vs endereço absoluto nativo.** *Resolução (não é gap):* provado
...
T-B5 entrega a resolução do interpretador COMPLETA e o encoder bridge nativo COMPLETO, provados
...
- **Migrar os descritores ABI** (`SYSV64`/`AAPCS64`/`WIN64`, `UPPER_SNAKE`
  ...
  Tier-B real lido idêntico no interpretador e no nativo. Ambos são T-B6, atrás do BUMP #3. Nada
