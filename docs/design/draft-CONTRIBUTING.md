...
- **Green `mc build ngen && run`**: all 18 fixtures compile and execute with correct exit codes
  on your platform.
...
- **No new intrinsics or hardcoded backend logic.** Every function has surface code (`exp fn` in `.tk`).
  If a feature "wants" special handling in the backend, that's a fork — open an issue in DECISION_LOG.
...
- **Refused features carry `teko:` prefix:** v0.1.0 does not support `Func<>`, `params T[]`, `T[][]`,
  nested `namespace`, `Variant`, etc. Any of these triggers `teko: <cause short>`.
...
2. Check `docs/design/port-teko-mc.md` and `ngen/HANDOFF.md` (§2, the laws).
3. If genuinely open, open an issue in `DECISION_LOG.md` (assign to owner if urgent) with the fork
...
- **`ngen.yml`**: matrix of 5 native legs, each runs `mc build ngen` and all 18 fixtures.
...
For the full operational guide, see `ngen/HANDOFF.md`. For design decisions and the port rationale,
