...
Status: DESIGN (architect). Base: `origin/fix/retirement` HEAD `5732a1c6`. Design-doc only — NO product
code is written here. This is the crumb spine an implementer resumes from.
...
Two law-first options (both remove the pthread/glibc-TLS dependency; recommend **B**):
...
**Transitional posture (keeps every crumb landable while B is designed).** Until the seam is migrated to B,
the C leg can KEEP `_Thread_local` working on spawned threads by passing `CLONE_SETTLS` with the child's TLS
pointer (Option A, minimal) — because on the C leg glibc IS linked and its `__set_thread_area`-compatible TCB
can be borrowed from the parent's allocator. That lets `thread_clone` (M1) land and the fixtures go green on
the C leg BEFORE the seam rewrite, then B lands as its own leaf crumb and the native leg inherits it. The
implementer picks A-transitional-then-B, or B-first; **the owner ruling requested in §10 is: A-then-B, or
B-first, or a third scheme.** This is the one genuine fork — flagged, not silently chosen.
...
5. **M4 per-thread seam → tid-table (Option B, §5)** — `[C]` if it re-shapes `tk_arena_control_get/set` in
   `teko_rt.c` + the seam accessor, `[L]` for the Teko table. Gated on the §5 owner ruling. Sequence AFTER
   M1/M3 land on the A-transitional TLS so spawn is already green; M4 then removes the glibc-TLS coupling.
...
- **R1 — HEADLINE / GENUINE FORK (HALT): the `_Thread_local` seam has no backing on a raw `clone` thread.**
  glibc TLS (`%fs`/`tpidr`) is not set up by raw `clone`, so `tk_g_current_task` / arena control corrupt or
  fault on a spawned thread (§5). RESOLUTION options are law-first and ratifiable, but WHICH one is an
  architecture choice the owner should set because it shapes `task_begin`/`task_end` and the arena seam:
  **(A)** hand-lay a TCB + `CLONE_SETTLS` (re-imports glibc ABI — rejected as primary), **(B RECOMMENDED)**
  replace the seam with a `SYS_gettid`-keyed per-thread table in program-region memory (fully
  libc-independent, native-leg-identical), or **(A-transitional-then-B)** land spawn on minimal `CLONE_SETTLS`
  so M1/M3 go green on the C leg, then migrate to B as M4. **This is the single ruling needed before
  implementation.** Recommendation: A-transitional-then-B. Everything else in this plan is decided law-first
...
No tension beyond R1 is unresolved. R1 is a genuine architecture fork (not a law violation) — **HALT for the
owner ruling on the per-thread-state scheme (A / B / A-then-B) before M1 implementation begins.** Everything
up to and including the M0 numbers and the M1 intrinsic SHAPE is design-complete and law-clean; the
implementer can land M0 immediately and scaffold M1's helper against the recommended A-transitional path while
