...
> intactos. **encode_*/ELF/Mach-O/COFF/interpretador intocados; sem seed bump** (o 🔑 SEED BUMP #3
...
1. **A resolução é BARATA e testável à mão.** Ao contrário dos encoders nativos — que
   precisam de um registro de relocation que os writers/interpretador não consumiam até
...
2. **O caminho wasm é self-contained e fica COMPLETO com T-B4.** wasm não depende de
   linker, writer nativo, nem interpretador. Implementar a resolução real fecha o wasm32
...
§2.4 encadeia. Roda idêntico no interpretador e no harness nativo (é um `.tkt` de backend).
...
> patcheado. Nenhum encoder nativo, writer ELF/Mach-O/COFF, ou interpretador é tocado (esses são
...
- **RITUAL POINT — fim de T-B4:** gate COMPLETO — todos os goldens wasm byte-idênticos
  ...
  ambas as engines (interpretador + nativo) + 100% de cobertura do delta (§5). **Sem seed bump** —
...
- **T-B5 (interpretador):** resolver o ponteiro rodata-interno no `LGlobalAddr`/load tipado do
