...
**Ritual (all):** full gate — gen1 `teko . -o bin` (native #test) + `./bin/teko test .` (the interpreter) + FIXPOINT gen1==gen2 byte-identical + `diff_c_own.sh` + `TEKO_MEM_PARANOID=1` + `//`-audit.
...
## A/#301 — closure-in-Ref/optional round-trip — **verified 5 native sites + 1 interpreter probe**
...
**Crumb 2 (the interpreter) — the ref-cell closure round-trip — PROBE-GATED (design complete, one line probe-pinned).**
Verified interpreter facts: `eval_lambda_call` (`the interpreter`) DOES merge callee cells back (`with_cells(caller, fe.env.cells)`, `:2981`); the deref-read `cell_get` (`the interpreter`) and cells-merge for a user fn (`:3267`) are `Value`-generic, so a `FuncVal` threads through structurally. Two verified suspects:
1. **snapshot capture** — `fv.cap_vals` bound by value at `eval_lambda_call:2977` (from the `TLambda` snapshot at `the interpreter`): a closure that must observe a cell WRITE made after its capture needs to capture the CELL INDEX (a `RefVal`), not the value. This is already the sound pattern (#300's `make_counter` proof).
2. **`coerce_to` on a Func return** — `the interpreter` runs `coerce_to(rv, f.return_type)` on the returned value; if `f.return_type` is a `Ref<Fn>`/`Fn?` the coerce may not carry a FuncVal through unchanged. **Verify `coerce_to` has a Func/Ref-of-Func passthrough.**
...
**Fixtures:**
- `examples/regressions/closure_in_ref_roundtrip/` (interpreter==native) — the `reseat` repro; exit-code protocol.
...
**Ritual:** ship BOTH engines in ONE PR (mangle=native, cell=interpreter) so `diff_c_own.sh` never sees a half-fixed state. Corpus carries no `Fn?`/`[]Fn` today → gen1==gen2 trivially preserved (guard: the new arms are a no-op unless a Func inner appears).
...
- **#301 interpreter crumb 2** — probe-gated: `TEKO_TRACE` the `reseat` repro to pin snapshot (`the interpreter`) vs `coerce_to` (`the interpreter`) vs merge. Design complete (capture-the-cell-index). Native crumb 1 (mangle) is fully specified and unblocks the `ReadFn | error` union-arm case immediately.
