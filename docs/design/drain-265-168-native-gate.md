...
the interpreter — it compiles to a NATIVE binary that knows the product-under-test + its access
points; interpreter stays dev/debug only). This is THE shared-villain kill: it drops the
compiler's gate-time peak from 1595 MB → ~366 MB (the child runs in its own process) AND
the 15.6 s interpreter interpret → a sub-second native run. Both axes bank it once.
...
- **Function-level marks.** `emit_function_mode` (`codegen.tks:7659`) → `emit_function_cov`
  ...
  `is_test` + interpreter-internal items), keyed by `fn_items_index` (`codegen.tks:7674`). `cov_idx < 0` ⇒
...
- **Orchestration.** `run_native_gate` (`project.tks:734`) emits the TestCov TU → `run_cc`
  → run child → the interpreter's `cov_merge(covfile)` (→ `tk_cov_merge`). `run_gate`
...
- **Engine-independent floors.** `line_coverage`/`branch_coverage`/`functions_coverage`
  (`the interpreter`) statically walk each production fn body (`cov_walk_block`,
  `the interpreter`) and query `tk_cov_line_hit(i, line)` / `tk_cov_branch_hit(i,line,col,o)`
...
sinks INSIDE production fn bodies. The interpreter populates them at three AST sites:

- `eval_expr` head: `teko::cov_line(e.line)` on EVERY evaluated expr (`the interpreter`).
- `exec_if`: `cov_branch(e.line,e.col,0)` then-taken / `(…,1)` else-taken (`the interpreter`).
- `exec_match` / value-match: `cov_branch(e.line,e.col,i)` for arm `i` (`the interpreter`).

So `native_function_floor` (`project.tks:719`) enforces ONLY the function floor; line+branch
floors are left to the interpreter lane. **Closing #265 = emit those `tk_cov_line`/`tk_cov_branch`
calls in the TestCov codegen so native enforces ALL THREE floors → the interpreter lane can leave PR CI.**
...
(`emit_test_call` cov branch). That is CORRECT for the interpreter (whose `eval_call` does
`cov_enter(h.idx)` for the CALLED production fn too — `the interpreter`), but the native
...
**Resolution (mirror the interpreter exactly):** every PRODUCTION fn body, under TestCov, must be
bracketed `tk_cov_enter(<own items-index>)` … `tk_cov_leave()` around its body, so its interior
`tk_cov_line`/`tk_cov_branch` marks key on ITS index (matching the static walk's `i`). The interpreter
does this in `eval_call`/`call_value` (enter on entry, leave on return, `the interpreter`).
...
the sub-second win). A is the correctness prerequisite for demoting the interpreter lane; B is the speed
...
The interpreter marks on EVERY `eval_expr`; naively mirroring that is a mark per sub-expression, which
bloats the TU and is unnecessary — the floor walk (`cov_walk_expr`, `the interpreter`) collects the
...
carries a `line` on each; the interpreter reads `e.line` off the expr, so prefer the statement's primary
...
Three emission sites, each mirroring one interpreter `cov_branch` call:
...
- `emit_if_stmt` (`codegen.tks:5288`): after `if (<cond>) {\n`, emit `tk_cov_branch(<line>,<col>,0);`
  ...
  outcome-1 mark exists — the interpreter marks outcome 1 on the SKIP path too, `the interpreter`) emit
 ...
 * taken branch outcome (the interpreter's exec_if/exec_match cov_branch twin). No-op when off → byte-identical.
 *
 * @param buf     the emission buffer
...
`cov_addsite(w0, e.line, e.col, m.arms.len)` (`the interpreter`). The subject is evaluated once
...
- **Recommended (exit-edge-proof):** do NOT use enter/leave for production bodies. Instead pass
  ...
  `codegen.tks:2075`-area (the same `else if last == "cov_line"` dispatch block). The interpreter continues
...
already reads the interpreter's `line_coverage_pct`/`branch_coverage_pct`/`coverage_pct` off the merged sinks.
After A2–A5 the native run populates all three sinks, so `coverage_gate_result` (the interpreter path's own
...
the DOUBLING the compile-time doc names is at the CI level (interpreter gate + native gate both run). The
...
memory balloon (1595 MB) is the interpreter interpreting tests IN-PROCESS. The native gate already runs the
tests in a CHILD process (`teko::process::run`, `project.tks:748`) → the compiler's own peak drops
to the codegen-only 366 MB the moment the interpreter gate is no longer the default. This is Phase B's core
...
**C1 — Make native the DEFAULT gate; demote interpreter to a periodic lane.** After A6 (native enforces all
...
`native.yml`: drop the interpreter-gate test step (`native.yml:82`) from the 5 build-test platforms;
keep ONE nightly/periodic lane running `teko test .` (the interpreter) over the full corpus for interpreter regression
(#265 governance: the interpreter lane stays as a SEPARATE CI job, not per-PR). `diff-interp-native`
(`native.yml`) keeps proving interpreter==native on the 79 `examples/regressions/` fixtures. This realizes
the 16m→~5-6m + the memory win. **This is CI-config, gate-able independently, LOW risk once A6
proves the three native floors match the interpreter floors on the fixtures (§4 A-parity fixture).**
...
(engine-independent — they already work once the sinks are populated). The interpreter's own
`cov_line`/`cov_branch` marking (`the interpreter`/…`) is UNTOUCHED (interpreter lane unchanged).

## 4. Regression fixtures (inputs → expected exit codes)

Add under `examples/regressions/` (the `diff-interp-native` harness set) + `.tkt` gate fixtures. Each
runs on BOTH engines during the transition (the diff harness stays), and the NATIVE exit is the
authority per the interpreter=dev/native=production ruling.

1. **cov_line_native_parity** — a fn with N distinct executable lines, a `#test` exercising a
   subset; assert `line_coverage_pct(native-merged) == line_coverage_pct(interpreter)`. Native exit 0.
...
8. **A-parity (the Track-C unblock gate)** — over the 79 `examples/regressions/` fixtures, assert
   `{functions,lines,branches}_coverage_pct` are EQUAL interpreter-vs-native. This is the fixture the CI flip
   (C1) waits on: only when native==interpreter on all three floors across the corpus may the interpreter lane leave PR.
...
- **After A5** (all instrumentation + `_at` keying): FULL gate on a project WITH `--native-gate`;
  the native run must populate line/branch sinks and the floors must MATCH the interpreter (fixtures 1–5).
...
- **Before C1** (CI flip): fixture 8 (A-parity) GREEN across the 79 fixtures — the mandatory
  precondition to demote the interpreter lane (main-integrity: never drop a floor that catches a regression).
...
native gate's coverage MERGE (the interpreter's `cov_merge`) is order-independent (dedup'd id sets) → no ordering
...
`tk_line_id`/`tk_branch_id` packing the interpreter uses (A2–A5 emit the same `(fn,line)` / `(fn,line,col,
outcome)` tuples the interpreter marks). Fixture 8 proves EQUALITY before the interpreter lane is demoted → the floor
...
- **R4 (double cc cost, LOW).** The native gate cc's a second TU (test) alongside the release TU.
  #249 measures cc at 0.7 s / 2.4% — negligible; the net is still a massive win (kills the 15.6 s interpreter
  interpret). Documented in B1; no action.
- **R5 (law tension — main-integrity vs demoting the interpreter lane).** `teko-main-integrity-absolute`:
  never weaken the gate so a regression slips. Demoting the interpreter lane (C1) is safe ONLY if native
  enforces EQUAL floors. **Resolution: fixture 8 (A-parity across 79 fixtures) is the mandatory
  precondition; the interpreter lane stays as a nightly job (#265 governance) so interpreter regressions are still
  caught. No genuine tension remains — the laws RESOLVE it (equal-or-stronger floors + retained interpreter
  nightly).** Not a HALT.
