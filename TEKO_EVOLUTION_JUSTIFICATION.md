...
> compilation units**, **49 `.h`**, **14 `.tkt` tests** — **63 interpreter-run units (`.tks`+`.tkt`)**, **138
...
| `.h` headers | 49 | `src/**/*.h` |
| `.tkt` interpreter tests | 14 | `src/**/*.tkt` |
| interpreter-run units (`.tks`+`.tkt`) | 63 | the "≈64 corpus files" |
| subsystems | 11 | `src/{lexer,parser,checker,codegen,interpreter,emit,text,build,runtime,assert,…}` |
...
- **Self-host unblock — the headline.** **212** `teko::list` calls + **98** `[]T as x` patterns + **307**
  ...
  self-compile** until the collection read-side + COPY-append are lowered through codegen/interpreter. Per
...
- **Numbers.** M:N green threads give cheap spawn (thousands of tasks, ~KB stacks) vs. 1:1 OS threads
  (~MB stack, syscall to clone). For the compiler's actual parallel unit — **49 `.tks` / 63 interpreter-run
...
- **The compile-time speedup [projected].** The checker/lowering passes over the **49 `.tks` source
  units** (or 63 interpreter-run units) are *embarrassingly parallel per file* up to the cross-file resolve. Model:
...
| **2** | **Collections (copy-append + read-side)** | Unblocks self-host of **~55% of corpus**; 212 calls / 98 patterns / 307 `.len` lowered; frozen forward-compatible surface | Medium-High (codegen/interpreter slice subsystem) | self-host backend only | **Top** — gates everything; mandatory |
...
then drive the collections read-side + COPY-append through codegen/interpreter to finish self-host.** Collections is
