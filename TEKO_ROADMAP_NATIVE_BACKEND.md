...
- **WASI (`wasm32-wasi`):** `teko_rt` cross-compila com `clang --target=wasm32-wasi`
  ...
  funcional total com os outros 4 alvos, sem redesenho.
...
os 6 alvos da matriz (4 nativos + Wasm/WASI + Wasm/Browser). Dado o tamanho, M1 se
...
| **N7** | CI: novo workflow (ou extensão do `native.yml`) roda a suite via o backend nativo em paralelo ao backend C, nos 6 alvos (4 nativos + Wasm/WASI + Wasm/Browser) — todos precisam concordar (diferencial de 3 vias: VM == native-C == native-obj, os dois sub-alvos Wasm inclusos). | Gate CI verde, todos os motores concordando. |
...
objeto-de-outro-alvo no host de dev. **Wasm (os dois
