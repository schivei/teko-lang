...
TR1 honest stop; `codegen`+`interpreter` — a trait decl emits NOTHING (members live in the derivers after the
...
contextual identifier) + `examples/regressions/trait_basics` — interpreter == native, gen-2 == gen-3, 632 tests
both engines.
...
existing type/emit path handles them (zero new codegen, zero new interpreter node, interpreter==native automatic). `eq`
...
fixtures (`trait_derive_eq/hash/ord/clone_default/nested`, interpreter==native), a derived `eq`/`hash` matches a
hand-written one.
