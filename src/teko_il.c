...
void teko_il_serialize_binary(const BytecodeBuffer* buffer, const char* filename) {
    ...
    if (!file) {
        fprintf(stderr, "[Erro de Sistema]: Não foi possível criar o arquivo de saída %s\n", filename);
        return;
    }

    // 1. Monta e escreve o Cabeçalho Físico (Header)
    ...
    header.definitions_count = 0; // Calculado semanticamente em fases posteriores
    header.code_section_size = buffer->size;

    fwrite(&header, sizeof(TekoFileHeader), 1, file);

    // 2. Gravação do Constant Pool (Pool de Strings e Números Arbitrários)
    // Para cada string, gravamos primeiro seu tamanho em uint32_t e depois os caracteres brutos
    ...
    // 3. Gravação da Seção de Código (Opcodes e Argumentos da ISA)
    if (buffer->size > 0 && buffer->code) {
        fwrite(buffer->code, 1, buffer->size, file);
    }

    fclose(file);
    printf("[IL Codegen]: Arquivo binário independente '%s' gerado com sucesso (%d bytes).\n", filename, buffer->size);
}