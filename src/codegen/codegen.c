...
static char *cg_format_c(const char *src) {
    ...
    bool blk[4096] = {0};        // per-`{`: is it a function/control BLOCK (preceded by `)`)? — for top-level separation.
                                 // zero-init: a `(`/`[` frame (never written) reads `false` (not a block) instead of
                                 // uninitialized garbage — defined behavior even on unbalanced input (which codegen never emits).
