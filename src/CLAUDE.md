...
- `codegen_metal.c` — the polymorphic router: `teko_metal_emit_program(ctx, bytecode, size)`
  ...
  `ICONST/SCONST/JMP/JMP_IF_FALSE/FUNC_BEGIN/CALL_IMPORT`.
...
- `OpCode` enum lives in `codegen_li.h`; `0x09` = `OP_CALL_IMPORT` (Browser FFI); free
  slots include `0x0A–0x0F`, `0x15–0x1F`.
...
- FFI (`parser_ffi.c`) parsing is hardened (zero-init). The WASM backend now lowers an
  `extern … from "ns" as "name"` to a `(import …)` + `OP_CALL_IMPORT` (Browser FFI MVP-1);
  the parser→IL wiring for real `.tks` source is the remaining Browser FFI work.
