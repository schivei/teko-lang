...
> **Contenção do que já existe:** o `Value` variant do interpretador e o tipo `Unit` ficam marcados como **NÃO-CONFORMES**;
...
- **[Z2c] Interpretador** — deps: Z1, **Z-design** · par: twins C/Teko do interpretador
...
- **[Z3-codegen/interpretador] Repr. de opcional** — deps: Z3, Z2b, Z2c · par: `codegen.*`, `interpretador.*`
  > Baixar `T?` (presença + valor; niche/hasbits = otimização evolução) no nativo e no interpretador.
...
- **[Z-design] RESOLVIDA — modelo `Value` do interpretador.** Membros de variant = tipos completos (qualquer nativo,
  ...
  (largura/sinal); o espelho `.tks` usa a forma acima. *(floats/`error`-como-valor entram quando o interpretador crescer.)*
- **[Z3 escopo] RESOLVIDA = (a) COMPLETO.** `T?` entra na semente com suporte pleno (type-former embutido,
  não exige generics): modelo de tipos + parser (`?`/`?.`/`??`) + checker + codegen + interpretador. A regra "variant não
...
(decimal 256×256 = 512 bits) · `bigint` (precisão arbitrária) · `bool` · `byte`. *(O `Value` do interpretador e o
...
Crumbs (transversal: lexer → `PrimKind`/`scope` → checker → codegen → interpretador → runtime):
...
- **[N1] inteiros 128** — `PrimKind += U128/I128`; `tk_builtin_type` += nomes; **valor de literal e portador do
  `tk_value` widened p/ 128** (`__int128`); AST `number` cabe 128; codegen `__int128`; interpretador 128; guards F3 (÷0/cast)
...
- **[N2] floats** — `PrimKind += F16/F32/F64` (kind float); **literais float no lexer** (`.`/expoente — NOVO);
  checker (tipagem/inferência/conversões); codegen (`_Float16`/`float`/`double`); interpretador (kind float + aritmética).
...
paridade com a guarda de cast inteira). *(O interpretador pode aproximar f16/f32 via `double`+largura (modo debug); o nativo usa
...
são baixados** — codegen e interpretador os recusam ("not yet supported"). Logo `when` nunca executa. Crumbs:
...
- **[ME-interp] Interpretador**: interpretar os mesmos — `if`-valor, `match` (discriminar a tag do `tk_value`/variante +
...
- **W1a** twin Teko do interpretador: ~17 padrões-construtor `Type(x) =>` → `Type as x =>` (linhas 184–197, 314–337). [a queixa direta]
- **W1b** twin Teko do interpretador: `match <bool>`/`match try_builtin {…}` → `if` (try_builtin retorna bool → use `if`). Aninhamento triplo (l.378–383) → achatar com `when`/early-return.
...
**Onda 4 — Camada de VALOR struct/variant (codegen + interpretador) — A FUNDAÇÃO:**
- **W4** representar valores de `struct` (campos nomeados) e `variant` (discriminante + payload) em C e no `tk_value`; construção; **field access** (interpretador). Destrava W5/W6.

**Onda 5 — Execução de controle (codegen + interpretador):**
...
**Onda 6 — `T?` lowering (codegen + interpretador):** representação presença+valor; `?.`/`??`.
...
1. **Qualificação de namespace OBRIGATÓRIA.** Um tipo/símbolo de OUTRA namespace deve ser referenciado por
   **caminho absoluto** `teko::checker::TBinary` **ou** por `use teko::checker` (liga `checker`) + `checker::TBinary`.
   Bare só para a MESMA namespace. Hoje: twin Teko do interpretador usa `TExpr`/`TBinary`/`Path` bare; **zero `.tks` têm `use teko::`**;
...
- **[W4]** camada de valor struct/variant (codegen+interpretador). **[W5]** match/if-valor/`when`/desconstrução (codegen+interpretador).
...
7. **Nomes errados** (`TStr`→`TStrLit`, `TLoop`→`TLoopStmt`, `TByte`→`TByteLit` no twin Teko do interpretador) → correção semântica à parte (flag, não auto-fix no sweep mecânico).
...
> **Diferencial (M.1):** interpretador e codegen devem concordar (e o futuro backend nativo). W4a é upstream comum dos dois. Ordem: **W4a (parse+check+tast+tkb) → valida self-parse do corpus → W4b (interpretador) + W4c (codegen) em paralelo (diferencialmente equivalentes) → W5**.
...
> **Gaps registrados:** W2-tkb-mirror (tkb .tks faltam tags 12-15, pré-existente, #45); if-as-value trailing-return type mismatch (pré-existente, #46). **Próximo: W4b (interpretador) ∥ W4c (codegen)** — execução da camada de valor, diferencialmente equivalentes.

### §9.2 — W4b ∥ W4c FEITO + validado (camada de valor struct, dois backends concordam)
- **W4b (interpretador):** `tk_value` ganhou `TK_VAL_STRUCT { type_name; fields(names/vals) }`; eval de `TStructInit` (constrói em ordem declarada) + `TFieldAccess` (lê campo por nome). `error` é struct-value (type_name "error"). C + twin Teko do interpretador.
...
- **Validação diferencial (B.39):** `m::P { x=7, y=2 }; return p.x` → **interpretador exit 7 == binário compilado exit 7**. C verde; C gerado compila+roda.
...
**Descoberta (gap compartilhado):** ambos os backends capturam o valor de uma fn SÓ via `return` explícito — o **valor da última expressão (return implícito)**, incl. `if`/`match`, é descartado (interpretador `tk_vm_exec_block`/call; codegen idem). W5-exec precisa fiar o valor-de-bloco / return-implícito.

**W5-exec (task #50) — execução, nos DOIS backends, diferencialmente equivalentes:**
- return-implícito / valor-de-bloco (interpretador + codegen).
- interpretador: eval de `TK_TEXPR_IF`/`TK_TEXPR_MATCH` como valor (roda o ramo escolhido, rende o valor) + motor de match (literal/range/Alt/caso-de-variant/desconstrução-de-campo + guards `when` + bindings). Hoje `vm_unsupported`.
...
### §12.1 — W5a (interpretador): if-valor + return-implícito + chamada cross-ns — FEITO + validado
...
- **Fix cross-ns:** `find_function` resolve pelo ÚLTIMO segmento (era só single-segment) → chamadas `ns::fn` funcionam no interpretador.
- **Validado (`teko run`):** `if`-valor em binding (7/8); `if`-valor trailing no main (7/8); stmt-if + mutação + trailing (9); `if` aninhado (5); chamada cross-ns `m::two()` (2, return implícito) e `m::pick()` (7, if-valor). C verde + twin Teko do interpretador.
> Resta no W5-exec (task #50): **codegen** if-valor (statement-expr/hoist) + **match** (motor de pattern-matching: literal/range/Alt/variant-case/desconstrução + `when` + bindings) em interpretador **e** codegen; depois prova diferencial interpretador==binário. Gaps relacionados: literal trailing não adapta ao tipo de retorno (`-> i32 { 2 }`); params de fn no interpretador (honest-stop); codegen cross-ns fn mangling (#49).
...
- **if-valor**: lowering como statement-expression GNU `({ <T> _tkN; if (c) {…_tkN=then;} else {…_tkN=else;} _tkN; })` (clang/gcc aceitam sob o `cc` do projeto); cada ramo atribui o valor trailing a `_tkN`; `return`/`break`/`continue` num ramo divergem via C (igual ao interpretador). Trailing `if` em posição-tail emite como control-flow direto (`if (c) {…return X;} else {…return Y;}`).
- **Fix cross-ns (#49 resolvido):** a chamada lowra para o ÚLTIMO segmento do path (nome bare), casando o decl bare de `emit_function` E o `find_function` do interpretador. Ambos backends = último-segmento → diferencialmente iguais.
- **Validado:** 12 casos diferenciais interpretador==binário (if-valor trailing, em binding, return-de-if, dois temps distintos, aninhados, loop+break, void trailing). C verde + mirror `codegen.tks`.

### §12.3 — W5b (interpretador): motor de match — FEITO + validado
...
> Resta no W5-exec (task #50): **codegen** do match (dispatch por tag + wrapping de valor-variant, camada deixada como fail_node no W4c) — em andamento (agente) com prova diferencial interpretador==binário; mirror `.tks` (interpretador/resolve/typer/match) em andamento (agente).
...
- **Descoberta:** if-sem-else / else-if / encadeado e loops aninhados (break/continue/return/if-profundo) JÁ funcionavam — validados diferencialmente (interpretador==binário).
- **W5-cf-1 — bracketless if/else (#51) FEITO:** `if cond` / `else` sem `{}` quando o corpo é **uma** statement na PRÓXIMA linha (separador-gated). Vira um bloco de 1 elemento → checker/interpretador/codegen tratam igual a um bloco com chaves. Bônus: `else` pode ficar em linha própria após um bloco com chaves. parse_if.c + parse_if.tks. Validado diferencial (then/else/valor/dangling-else/sem-captura-de-irmã).
- **W5-cf-2 — labeled loops + break/continue rotulados (#52) FEITO:** **SINTAXE (decisão do legislador — sinalizar p/ veto):** `loop NOME { … }` (IDENT logo após `loop`), `break NOME` / `continue NOME` (IDENT na MESMA linha; bare = loop mais interno, inalterado) — postfix, sem sigilo, inequívoco. Um break/continue rotulado borbulha pelos loops internos até o loop cujo label casa.
  - Camadas: ast.h (`label` em loop_stmt; `tk_jump{label}` p/ break/continue) · parse_stmt.c · tast.h · typer.c (carrega label + **validador `check_labels`**: jump rotulado deve nomear loop ENVOLVENTE; jump bare deve estar em loop; labels únicos por corpo — senão colidem os goto-labels do C) · twin C do interpretador (`tk_flow.label` + matching/propagação no loop) · codegen.c (loop rotulado emite `tk_lbl_<N>_cont:`/`_break:`; `break N`→`goto …_break`, `continue N`→`goto …_cont`).
  - Validado: happy-path diferencial interpretador==binário (break/continue p/ loop externo, triple-nested break-to-top, bare-innermost preservado, combinação com bracketless-if). Erros LIMPos (Teko, não erro do C): label desconhecido, break/continue fora de loop, label duplicado, break p/ irmão não-envolvente.
  - Mirror `.tks` (ast/parse_stmt/tast/typer/interpretador/codegen) em andamento (agente).
...
**Pipeline (projeto):** ler `.tkp` → **carregar pacotes (`.tkh` [+ `.tkb` guardado])** → discover+assemble do dev → **check (collect semeado com as interfaces das deps)** → **pré-link (funde `.tkb`)** → codegen|interpretador.
...
**Diretriz:** o conjunto de features do seed C deve ser parseável/checável/compilável pelo bootstrap, para o corpus `.tks` (64 arquivos) se auto-hospedar. Método: iterar `teko run .` (o compilador sobre o próprio `src/`), achar cada gap (parser/checker/codegen), implementar em TODAS as camadas (C + mirror `.tks`), validar diferencialmente (interpretador==binário), repetir. **Achado-chave:** "compilar Teko com Teko é o teste real do compilador C" — os bugs do compilador (ex.: o heap-corruption do `tk_env_define`) afloram aqui.

**Gaps fechados (cada um diferencial interpretador==binário):**
...
- **Bug de heap (tk_env_define) corrigido** + regressão committada (`examples/regressions/match_pattern_bindings`, interpretador=5==bin=5).

**Em andamento:** (gap #7 do march) **interpolação de strings `$"…{expr}…"`** (21 usos; precisa de runtime str-builder + int→str) — agente.

**Escopo grande restante (backend):** o subsistema **slice/list/`teko::list` stdlib** (codegen tem ZERO suporte a slice) e **parâmetros de função no codegen+interpretador** (toda fn do corpus tem params; ambos backends honest-stop em params hoje). São features multi-camada grandes — o front-end (parse+check) avança bem mais fácil que o backend (codegen/exec).

**Gaps fechados (cont.) — todos diferenciais interpretador==binário:**
9. **interpolação `$"…{expr}…"`** — lexer token + parser (re-lexa/parseia cada hole) + checker (holes str/int) + runtime (`tk_str_concat`/`tk_i64_to_str`/`tk_u64_to_str`) + interpretador + codegen + tkb (tag 18).
...
13. **subsistema de OPTIONALS `T?`/`null`/`?.`/`??` — FEITO** (era a parede do gap #13): repr de valor opcional (interpretador `TK_VAL_OPT` none/present; codegen `tk_opt_<inner>` typedef por-tipo); `null` literal + `null` PADRÃO de match (NONE) + arm present (bind do inner); present-wrap (`T`→`T?` em `assignable_to`/`emit_as`); `?.` (safe-field propaga none) e `??` (coalesce, short-circuit). Diferencial interpretador==binário (idioma `error?` ambos os arms, `??`, `?.`, `fn -> T? { null }`). Regressão `examples/regressions/optionals` (interpretador=bin=6).

**PAREDE atual (gap #14): `parse_expr.tks:115` — PADRÃO de slice `[]lexer::Token as ts`** → entra no **subsistema de SLICES**.

### §15.1 — SLICE value-layer **Increment A — FEITO + diferencial (interpretador==binário)** (task #57)
O **valor** de slice agora EXECUTA nos dois backends (era a montanha "codegen tem ZERO suporte a slice"). Survey multi-agente revelou que ~80% já existia (tipo `TK_TYPE_SLICE` completo no checker; `TK_VAL_LIST`+`v_list_empty`/`v_list_push` no interpretador, porém inalcançáveis). Implementado, **fixed+copy** (sem dynamic push — §16):
- **interpretador** (twin C do interpretador): `teko::list::empty`/`push` (copy-on-push) ligados no dispatch de builtins (`try_builtin_call`); índice de slice (bounds-check + `tk_panic_oob`, mesmo panic do nativo); `.len` de slice → u64.
...
- **Mirrors** (twin Teko do interpretador/`codegen.tks`): espelhados (agentes; divergências honestas: interpretador usa `try_list_builtin` separado pois o `try_builtin` do `.tks` é void-only; codegen usa `CgTypeSet` threaded-by-return; sentinel = `Void` no `.tks`).
- **Validado:** scratch `/tmp/slicetest` (empty/push/index/.len) interpretador=binário=62; regressões match_pattern_bindings(5) e optionals(6) intactas.
...
- **`[]T as x` (gap #14)** — padrão de slice no match: AST (`tk_bind_pattern` ganha `is_slice`/`slice_type`), parser (`parse_pattern_primary` reusa `parse_type_primary`), checker (`match.c` casa o membro slice + relaxa exaustividade), interpretador (`pat_match` arm de `TK_VAL_LIST`), codegen (test/bind do arm de slice). **Bloqueio acoplado:** exige codegen de variant `[]T | error` (membro slice → `emit_type_expr` ainda fail_node em posição de membro de type-decl) e/ou variant inline — entra no subsistema de **variants/retorno**.
- **literal `[]`/`[a,b]` + `xs+[x]`** — superfície alternativa (o corpus usa `teko::list::push`); baixa prioridade.
- **parâmetros de função em codegen+interpretador** — toda fn do corpus tem params; ambos honest-stop hoje (a outra montanha).
...
NOW (sem features de evolução; termina self-host, não tranca nada):
  ...
  [next]           slice-value layer + params de fn em codegen/interpretador + optionals  ← TERMINA SELF-HOST
...
2. Em seguida, retomar o self-host pela **slice-value layer + copy-append** (lado-leitura `[]T as x`/`.len`/índice já existe; falta repr de valor de slice em codegen+interpretador, literal `[]`, e append por cópia `xs + [x]`), tudo **fixed+copy** (sem dynamic push — diferido a S7). Cada incremento diferencial interpretador==binário.
