...
#
# GATED on linux-x86_64 (needs the host `readelf`/`objdump`/`ld` binutils
# reachable); on any other host the script HONEST-SKIPS with a named
...
echo "$hdr" | grep -qi "X86-64"        || fail "not an EM_X86-64 object"
