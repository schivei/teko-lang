...
**Interpreter retired:** the compiler's canonical source is Teko (`src/**/*.tks`), and the C23 bootstrap files (`src/**/*.{c,h}` except the runtime) are archived at tag `0.0.1.3-bootstrap`. All new work is written in Teko only; do NOT extend the frozen C bootstrap. The one exception is `src/runtime/teko_rt.{c,h}` (and `src/assert/assert.{c,h}`): the execution runtime linked into generated programs stays maintained C — it is the FFI seam for native binaries. CI seeds from the latest released `teko` binary.
