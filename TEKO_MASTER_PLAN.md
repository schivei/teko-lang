...
- **C-emission reps are provisional** — `ptr<T>`/`Ref<T>`/regions are shaped by the transpile-to-C backend; likely reworked at the future native-byte backend. **UPDATE 2026-07-01: now SCHEDULED** — full plan in `TEKO_ROADMAP_NATIVE_BACKEND.md` (matrix: Linux x86_64/arm64/riscv64, macOS arm64, Windows x86_64/arm64 — the exact 6-target CI matrix already in `.github/workflows/native.yml`; M1 is the complete feature set per target, not a toy slice; system linker (`cc`/`ld`) for M1, own linker deferred as L1–L4). Runs as a PARALLEL track alongside ROUND 2 (OOP CLASS, PR #39) — confirmed non-overlapping (`src/codegen/*` untouched by #39). Execution not yet started; this is the plan only.
