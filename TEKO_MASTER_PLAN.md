...
- 🔶 **Date/Time placeholder types** (intermediate step — user 2026-06-30: "passo intermediário antes de seguir com strings"). `DateTime=u128 ns`, `TimeSpan=i128 ns`, `Time=u64 ns`, `Date=i32 days`, `DateTimeOffset=struct{u128,i16}`. OS FFI: `clock_gettime`/`GetSystemTimePreciseAsFileTime` (resolution-fill, not precision-manufacture). Constructors `datetime_now()`, `date_today()`, `time_now_utc()`, `datetime_local_now()`. Namespace `teko::time`. *(agent running)*
- ⬜ **`b'…'` → `char` surface migration** — text-facing byte literals become `char`. Lexer stays byte-level; `byte` type remains real.
- ⬜ **Format interpolation** `$"{expr:F2}"` (static) + `$"{expr:[fmt]}"` (dynamic via `[…]` — user 2026-06-30: avoids `{…}` collision inside holes). Specs tied to value type: numeric F/D/X/N/E/G/B/P; Date/Time calendar patterns; `str` padding. Parser: `:` inside hole → Static(literal) or Dynamic(expr-in-brackets).
- ⬜ **Regex — compiled NFA** (`teko::regex`). `Regex("pattern")` compiles NFA at construction. Zero OS dependency. Supports `.` `*` `+` `?` `[]` `^` `$` `|` `()` `\d\w\s\b` `{n,m}`. API: `is_match`, `find->str?`, `find_all->[]str`, `groups->[]str?`, `replace`, `replace_all`.
- ⬜ **`str_from_utf8(bytes)->str|error`** — UTF-8 validation entry point (UTF-8-restricted declared-encoding).
