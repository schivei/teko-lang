...
| **W8** | **MEM-1a** ~~**`ptr<T>`** typed pointer~~ ✅ — `Ptr{inner: Type?}` (null inner = opaque `ptr` ≡ `ptr<void>` ≡ `*void`); resolve/type_eq/render/mangle/codegen (`<T> *`)/tkb; `ptr<i64>`→`int64_t *`; 200 tests both engines, fixpoint, clang-tidy-18 clean. **MEM-1b-i** ~~**`Ref<T>` TYPE plumbing**~~ ✅ — surface **`Ref<T>`** (capitalized, "the struct the compiler knows"); META type-case = `Reference{inner: Type}` (a DISTINCT name from the surface, like `Ptr`↔`ptr<T>` — capital surface names collide with internal type-cases in self-host codegen, so meta≠surface). Never null, no opaque form, mutable-target only per R2; reuses generics (no keyword), `Ref<T>` via resolve_generic_inst (rejects `Ref<void>`/bare); type_eq/subst/unify/collect/mangle(`ref_T`)/render(`Ref<T>`)/emit_type(`<T> *`, bare-pointer rep today)/emit_type_expr/monomorph/tkb (tag 13); 201 tests both engines, gen-2==gen-3 fixpoint, clang-tidy-18 clean. ⚠️ VM `type_eq` can't distinguish same-inner wrappers (descent bug — native correct via tags; [[teko-vm-wrapper-descent-bug]]). **MEM-1b-ii** ~~**Ref SEMANTICS (native)**~~ ✅ — `.value` read → `(*r)`; `.value` WRITE (`r.value op= x` → `(*r) op= x`) via a bounded `Assign.deref` target (parser `name.value <op>` → checker/codegen/tkb/monomorph/initanalysis); auto-ref on passing a `mut` lvalue → `&x` (codegen re-derives from the `Ref<T>` param; non-mut/non-lvalue rejected). Teko is value-functional (0 field/index assigns in the corpus) so **`Ref<T>` is the language's mutation-through-a-handle primitive**. Verified natively: `bump(r: Ref<i64>){ r.value = r.value + 1 }` on `mut x = 41` → caller's `x` becomes 42 (exit 84). **VM honest-stops** on any `Ref<T>` param (no wrong answer). 201 tests, gen-2==gen-3 fixpoint, clang-tidy-18 clean. **MEM-1c ✅ FOUNDATION COMPLETE (SECURITY-VETTED) — `Ref<T>` memory-safe by construction + runs in the VM == native + automatic dealloc.** Design panel + adversarial ASan red-team. **Step 1** ✅ `tk_region_drop` idempotent (`cf89a2e`). **Step 0** ✅ checker ESCAPE-GATE (`938b5b8`) — `Ref<T>` is PARAM-ONLY, SCALAR-ONLY; rejected as return/field/member/element/binding/`Ref<T>?`/generic-type-arg(written+inferred)/collection-literal; 2 red-team rounds closed real escapes. **Steps 2+3** ✅ (`602859e`) VM Ref aliasing via a cell store threaded through Venv+call path → `Ref<T>` runs in VM==native (statement-position; `bump(x)`→42). **codegen fix** ✅ (`906d66c`) void tail-match no longer falls through. **Step 4** ✅ (`5ca3e95`) per-function regions + conservative one-depth escape analysis (`escape.{tks,c,h}`) → real DEALLOC; 3 ASan red-team rounds closed 2 genuine UAFs (tail-return rebox; scalar-through-boxed-hop); 12 ASan reproducers + self-host-corpus-under-ASan clean. Memory-safety BY CONSTRUCTION: UAF (param-only ancestor target / escape→root), dangling (no escape typed), double-free (idempotent drop), null-deref (`Ref<T>?` banned), leak (frame regions freed). 230 tests, gen-2==gen-3, clang-tidy-18 clean. ⏸️ DEFERRED: value-position Ref in VM (honest-stops; needs eval_expr cell-threading); ⚠️ orthogonal pre-existing codegen bug found (`tk_opt_<T>` typedef not emitted for an unread optional self-ref field). *(deps: ptr<T> ✅, Ref ✅)* | `res`, `chk`, `cg`, `vm`, `rt`, `escape` | | 230 tests · gen-2==gen-3 · clang-tidy-18 · ASan-clean |
