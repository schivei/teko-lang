...
| **W10** | ⬜ **S4c** closures — RE-ORDERED HERE (need lifetime-bound capture storage): `fn` literals + lexical capture; env allocated in a region, referenced via `Ref<T>`; parser `\|params\| body`; checker capture types; codegen `{env, fn}`; VM closure value *(deps: MEM-1, S2 ✅)* | `ast`, `P`, `chk`, `cg`, `vm`, `tkb` | **1** | build green + VM==native |
