...
| **W9.3b** | ⬜ **ARENA MEMORY freed on PANIC/EXIT (user ruling 2026-06-29).** `defer` blocks SKIP panic/exit, but the MEMORY MODEL must not leak active regions at abnormal termination. Today `_tkfr`/`_tkbr` are stack-local C vars dropped only at NORMAL scope exits → a `panic(...)` / `teko::exit(n)` (diverging) skips them. FIX (rt + panic/exit emission, NOT defer): a GLOBAL intrusive registry of live regions in `teko_rt` (`tk_region_new` pushes, `tk_region_drop` removes, idempotent/NULL-tolerant) + `tk_regions_free_all()` drained by `tk_panic`/`tk_exit` before they abort/exit (and root at normal end). VM unaffected (regions codegen-only). *(deps: S2 ✅)* | `rt cg` | **1** | ASan-clean on panic + exit paths · gen-2==gen-3 | [[teko-defer-per-scope]]
