...
| **C7.16** `.tkb` statement/program codec — **THE KEYSTONE for packaging**. ⏳ near-complete (2026-06-27): (1) EXPRESSION codec complete+corrected; (2) `[]TStatement` + `TIfExpr` done; (3) **PROGRAM FRAMING DONE** — `serialize_program`/`deserialize_program` (version-2 frame) + `TypeExpr`/`Param`/`Field`/`TypeBody`/`TypeDecl`/`UseDecl`/`TFunction`/`TItem`/`TProgram` codecs, write/read/collect across BOTH twins (tkb_write/.c, tkb_read/.c, tkb_frame/.c); made `parser::TypeBody` + `checker::TItem` `pub`; +1 whole-program round-trip test; **169 tests, byte-identical**. **ONE REMAINING NODE: `TMatchExpr` (tag 9)** — its `TArm`s carry a `parser::Pattern`, whose `Literal`/`Range` patterns carry a `parser::Expr` (the untyped ~23-kind expr tree) → needs a `parser::Pattern` + `parser::Expr` codec (the final, self-contained piece; today serialize writes tag-9 stub, deserialize errors honestly). Unblocks C7.10 (pre-linker) + C7.12 (`.tkl` payload) | `tkb` | — |
