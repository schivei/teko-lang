...
| **W9** | 🔶 **S2 — block-scoped regions** (the remaining dealloc VOLUME). Per-FUNCTION regions DONE (W8). **LOOP-BODY regions ✅ DONE (`3751a0c`)** — per-iteration dealloc via `_tkbr<n>`, `tk_binding_is_block_local` (total fn reads == safe reads strictly inside B), full control-flow unwinding (break/continue/return drop the right region suffix); 252 tests both engines, gen-2==gen-3, clang-tidy clean, self-host + reproducers ASan-clean. **STATEMENT-position if/then, if/else, match-ARM regions ✅ DONE (`ac7b460`)** — `is_loop` flag (arm regions always crossed by break/continue, only loops are stop targets) + `emit_block_region`; also fixed a pre-existing twin-alignment bug (tail emitters passed empty `escaping`). **⬜ DEFERRED: VALUE-position arms** (an `if`/`match` yielding a consumed tail value) — the Teko twin's `emit_expr` is purely functional (no region params), so value-arm regions would need region+escape context threaded through ~65 `emit_expr` sites (not safely byte-mirrorable); value-arm bindings stay leak-safe (enclosing/root). **USER CLARIFICATION (2026-06-29): every function/method/BLOCK automatically gets its own arena scope, cleaned when it ends — structural, NOT annotation-driven.** *(deps: MEM-1 ✅)* | `chk cg escape` | **1** | gen-2==gen-3 · ASan-clean · +coverage |
