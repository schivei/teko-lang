...
| **C7.1f** `os()` builtin (✅ `tk_rt_os`) + `[extern]` cc/target/sysroot/freestanding (✅ — cross-build VERIFIED via x86_64-darwin/Rosetta) + musl portability fixes (✅ `execinfo` guard + `_GNU_SOURCE` in teko_rt.c); remaining: per-OS `[extern.search]`/`[extern.libs.<os>]`/static-shared + `#os(...)` conditional-compile | `build`, `L`, `P`, `chk`, `cg`, `rt` | C7.1a–e |
