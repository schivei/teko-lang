...
| **W8** | **MEM-1** **`ptr<T>`** opaque typed pointer (FFI-only, `#repr(C)`, `ptr ≡ ptr<void>`) + **`Ref<T>`** struct over it — the compiler-known "C pointer WITHOUT `*`/`&`"; deref/assign via `.`+`mut`; NO new keyword *(deps: type-generics ✅)* | `res`, `chk`, `cg`, `vm`, `rt` | | build green + VM==native |
