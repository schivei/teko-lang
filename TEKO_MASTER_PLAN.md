...
- 🔶 **OOP A1** (method model: receiver = 1st untyped param; struct=value/immutable, class=auto-`Ref`) — DONE for STRUCTS (PR #37, 2026-07-01): `type S = struct { fn a(self) -> R {…}; fn b() -> R {…} }`, instance dot-call + static `::`-call both resolve through the existing #41 namespace-qualified-call machinery (methods desugar to ordinary TCall, zero new codegen/VM code). CLASS methods (auto-`Ref<T>`, mutable) remain ⬜ — blocked on `class` existing as a construct (W10b.0.B still open).
