...
static int load_sources(int argc, char **argv, int first_input, TekoAllocator allocator, TekoSource **sources, size_t *source_count) {
    int i;
    size_t count = (size_t)(argc - first_input);
    ...
    for (i = first_input; i < argc; i++) {
        TekoString path = teko_string_from_cstr(argv[i]);
...
static void free_sources(TekoAllocator allocator, TekoSource *sources, size_t source_count) {
    size_t i;
...
static void print_diagnostics(const TekoCompileResult *result) {
    size_t i;
...
int main(int argc, char **argv) {
    ...
    int first_input = 1;
    TekoAllocator allocator = cli_allocator();
