...
void test_teko_lsp_protocol_handshake_and_routing(void) {
    ...
    // 1. Simula payload JSON-RPC de inicialização enviado pelo CLion/VSCode
    ...
    // 2. Simula requisição automática disparada no atalho de formatar documento
    ...
    // 3. Executa chamadas de escrita de diagnóstico simuladas (capturadas sem travamentos)
    teko_lsp_send_diagnostics(server, "main.tks", "Erro de Mutabilidade", 5);

    teko_lsp_destroy(server);
}

// Acrescente essas linhas de asserção dentro da função test_virtual_sdk_injection_and_isolation
void test_virtual_sdk_injection_and_isolation(void) {
    ...
    // Valida se o compilador agora encontra o resolvedor do sistema de arquivos e logs do SDK virtual
    ...
    // Garante que o isolamento do linter reconhece os novos caminhos como reservados
...
void test_vm_intrinsic_resolution_and_execution(void) {
    // 1. Valida se a string estendida do linter/parser bate com a resolução de IDs da VM
    ...
    // 2. Simula o ambiente físico mínimo para disparar a execução de uma intrinsic
    ...
    // Deve interceptar o comando e retornar o código de disparo com sucesso
...
void test_teko_cli_driver_assembly_mode_and_target_parsing(void) {
    // Simula: teko build projeto.tkp -S --target=riscv64-unknown-linux-gnu -o meu_asm
    const char* mock_argv[] = { "teko", "build", "projeto.tkp", "-S", "--target=riscv64-unknown-linux-gnu", "-o", "meu_asm" };
    int mock_argc = 7;

    // Simula a lógica interna do parser da CLI
    ...
    // Asserções do Unity para o Modo Assembly
...
void test_teko_cli_driver_object_mode_win_arm64(void) {
    // Simula: teko build projeto.tkp -c --target=aarch64-pc-windows-msvc
    ...
    // Asserções do Unity para o Modo Objeto e Windows ARM
...
void test_teko_cli_driver_default_host_fallback(void) {
    ...
    // Garante que o target padrão permanece idêntico ao host físico detectado (Mac Apple Silicon)
