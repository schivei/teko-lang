...
void test_teko_bare_metal_target_detection_and_parsing(void) {
    // 1. Valida a auto-detecção no hardware local (Mac Apple Silicon rodando Darwin)
    ...
    // 2. Teste de cross-compilação para a arquitetura aberta RISC-V de 64 bits
    ...
    // 3. Teste de cross-compilação para WebAssembly voltado a runtimes Web/Cloud (WASI)
