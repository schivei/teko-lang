...
### D68 · RATCHET DE MEMÓRIA — toda mudança BAIXA o pico, ou é trabalho à toa (owner 2026-08-24) ✅ RATIFICADA
Durante o processo de ajuste de memória (Eixo A/B/C, jardinagem, DPS), toda mudança tem que **BAIXAR o pico (estrito)** — **não basta manter**: uma mudança que não baixa não fez o trabalho da campanha, é **trabalho à toa** (repensar pra produzir queda, ou não landar). E o pico **NUNCA pode crescer**: qualquer aumento = regressão, reverter/corrigir ANTES de landar. Gravado em `CLAUDE.md` (bloco do guard de memória). Reforça o "grão a grão" (D-anterior/dono): cada passo baixa ao menos um grão; queda-zero é à toa.
