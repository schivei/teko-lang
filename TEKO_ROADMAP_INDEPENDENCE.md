...
| C3 | **Backend nativo próprio** — emitir direto ao metal + linker próprio, aposentando o `cc` do host (o transpile-para-C é degrau, não o design final). **Plano completo em `TEKO_ROADMAP_NATIVE_BACKEND.md`**; tracked como **ROUND N** (independente/paralelo) em `TEKO_MASTER_PLAN.md` (matriz Linux x86_64/arm64 + macOS arm64 + Windows x86_64 + **Wasm nos dois ambientes (WASI e Browser)**; riscv64 e windows-arm64 removidos da matriz em 0.3.1 por decisão do owner; linker do sistema no M1 (incl. `wasm-ld`), linker próprio como eixo L1–L4 futuro, Wasm fora do L1–L4 por já ter `wasm-ld`). | M.0 | **agendado** (plano fechado 2026-07-01, Wasm/WASI incluído no mesmo dia, escopo estendido a Wasm/Browser no mesmo dia; execução ainda não iniciada) |
