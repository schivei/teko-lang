...
- **[C3] Backend nativo próprio** — deps: C1.1 · M.0 · **agendado** · par: `src/codegen/native/*` (lir/isel/regalloc/enc/obj + `native_emit`, `src/runtime` inalterado como link target)
  > Emite direto ao metal, aposenta o `cc` (eventualmente). Plano completo (matriz de alvos, arquitetura de camadas,
  > milestones N1–N7, linker próprio L1–L4 diferido) em `TEKO_ROADMAP_NATIVE_BACKEND.md`. **Aceite (M1):** os 6 alvos
  > de CI (`.github/workflows/native.yml`) rodam a suite via objeto nativo, 3 motores concordando (VM==native-C==native-obj).
