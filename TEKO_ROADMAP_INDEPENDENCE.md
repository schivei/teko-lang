...
LEGISLATION §186–187)*. Perfis: **debug=interpretador**, **release=nativo**, **CI roda release** como portão de
...
4. **Testes NÃO são artefatos (legislador).** Um `.tkt` é lexado/parseado/checado como o resto, mas é
   **executado durante o build, no interpretador, sobre a árvore tipada — ANTES de qualquer emissão
   ...
   **[se há `.tkt`: interpretador roda os testes + mede cobertura → portão]** → emit/codegen. O portão **precede** a
   emissão. → **É por isso que o interpretador (Eixo D) entra agora**: ele é o motor de execução dos perfis
   **test/debug** (Stage-1 — CONSTITUTION §237–245). O runner/cobertura/portão (B3–B5) são realizados
   **sobre o interpretador** (ver Eixo D: D2/D3/D4).
...
| B3 | **Coletor + runner `#test`** → realizado como **D2** (roda no interpretador, não em nativo). `teko test`. | M.1 | → Eixo D |
| B4 | **Cobertura por LINHA** → realizado como **D3** (o interpretador conta linhas executadas; `.tkt` fora do denominador). | M.3 | → Eixo D |
...
> **Resultado do Eixo B + D:** `teko test` roda a suíte no **interpretador** (cobertura por linha medida); `teko build
...
*(l.1186–1187)*. Três estágios: **`.tkb` interpretador → AOT-nativo (LTS, é o que ships) → bare-metal**
...
genéricos + constraints · ponteiros de função / `use`-capture / `inject` (DI) · métodos/OOP (override pós-genéricos) · `flags` · pacotes/pré-linker (A6) · concorrência (`ref`+escape) · crypto/TLS (libs finas sobre FFI, **sem** TLS/HTTP nativo). *(Citações: REBOOT_PLAN §3–7, l.1104–1170; TEKO_HISTORY backlog.)* *(O "modo interpretador" saiu do backlog — promovido ao Eixo D, pois os testes exigem o interpretador agora.)*

---

## Eixo D — interpretador (perfil test/debug)

**Por que agora:** os testes (Eixo B) **executam no interpretador sobre a árvore tipada, antes de emit/codegen**, e são
descartados (não viram `.tkb`/binário). O interpretador é o motor do **sub-perfil de teste** — o **Stage-1** da doutrina
*(CONSTITUTION §237–245: `.tkb`/IL interpretado, o degrau de bootstrap)*.

> **Sub-perfil de teste (legislador):** é uma **fase de interpretador comum a todos os perfis**, não exclusiva de
> debug/test. **release TAMBÉM roda os testes** — executa o sub-perfil de teste **no interpretador** (o portão) e só
> **então** segue o pipeline para o codegen nativo. Resumo: **debug** = roda no interpretador (+ sub-perfil de teste);
> **test** (`teko test`) = só o sub-perfil de teste, isolado; **release** = **sub-perfil de teste no interpretador →
> portão → codegen nativo**. `--no-test` pula a fase em debug/test, **nunca em release** (havendo `.tkt`).

O interpretador interpreta a **árvore tipada (`tast`)** diretamente — para testes NÃO há passo de serialização `.tkb`
(o `.tkb` é só para pacotes/deps, A6). O mesmo interpretador é o motor que o release usa para o portão antes de emitir.
...
| D2 | **Runner sobre o interpretador** (≙ B3) — coleta `#test`, executa cada uma no interpretador **junto do código da sua namespace** (multi-arquivo via A3), reporta nome/contagem/pass-fail, **exit ≠0** em falha. `teko test`. | M.1 | dep: D1, A3 |
| D3 | **Cobertura no interpretador** (≙ B4) — o interpretador conta **linhas** executadas durante os `#test`; % sobre o código de produção (`.tkt` fora do denominador). | M.3 | dep: D2 |
| D4 | **Portão pré-emissão** (≙ B5) — o driver roda o portão do interpretador (testes + cobertura) **ANTES** de emit/codegen; release barra em falha/cobertura<threshold (configurável, piso 10%, default 80%); debug/test permite `--no-test`. | M.1 | dep: D3, A1 (threshold no `.tkp`) |

> **Resultado do Eixo D:** o interpretador existe como motor test/debug; `teko test` roda a suíte interpretada; o
> portão de release passa pelo interpretador antes de emitir qualquer artefato. O mesmo interpretador é o degrau para o
...
| E2 | **`teko::Error` + pânicos com file:line** — `tk_error`/`Error` ganham `file`+`line`; pânicos (÷0/cast/OOB/assert) imprimem `arquivo:linha` (no interpretador e no nativo). | M.1/M.3 | **novo** (dep E1) |
...
| E4 | **Stack-trace** — frames de chamada carregam origem; pânico imprime a pilha (interpretador direto; nativo via `.tsym`). | M.1 | **novo** (dep E2, E3) |
...
- **[B1! aderência do `teko::assert`]** `src/assert/` precisa do **par co-localizado** `assert.{tks,c,h}`
  (hoje só `assert.tks`; a impl C está em `teko_rt.c`/no twin C do interpretador/`scope.c`). Mover a realização C dos
  `teko::assert::*` para `src/assert/assert.{c,h}` (a casa da namespace; como os testes rodam no **interpretador**, é o
  interpretador/assert que a consome) — fechar a brecha de par.
...
- **B3/B4/B5 → realizados no Eixo D** (runner/cobertura/portão rodam **no interpretador**, não em nativo). Ver D2/D3/D4.

### Eixo D — interpretador (test/debug)
- **[D1] Interpretador da árvore tipada** — deps: — · M.0 · par: twins C/Teko do interpretador (+ testes correspondentes)
...
- **[D2] Runner `#test` no interpretador** (≙ B3) — deps: D1, A3 · M.1 · par: `src/build/testrun.{tks,c,h}` + `driver` (`teko test`)
  > Coleta `#test`, interpreta cada uma no interpretador **junto do código da namespace** (multi-arquivo via A3), reporta, exit≠0 em falha. **Aceite:** `teko test` roda a suíte do repo no interpretador; falha injetada → exit≠0 + relatório. (Fecha o `assert_test.tkt`.)
- **[D3] Cobertura por linha no interpretador** (≙ B4) — deps: D2 · M.3 · par: `src/build/coverage.{tks,c,h}` (contadores no interpretador)
  > O interpretador conta linhas executadas; % sobre produção (`.tkt` fora do denominador). **Aceite:** relatório determinístico para o repo.
- **[D4] Portão pré-emissão** (≙ B5) — deps: D3, A1 (threshold), A5 (driver) · M.1 · par: `driver` (`build --release`/`--no-test`)
  > portão do interpretador (testes + cobertura) **antes** de emit/codegen; release barra em falha/cobertura<threshold; debug/test permite `--no-test`. **Aceite:** abaixo do threshold ou teste vermelho → release barrado antes de emitir; `--no-test` recusado em release.
...
### Estado
- **✓ Feitos:** A1, A2, A3, B1, B2, M1, F3-pânicos, D1 (+ integração interpretador: `teko run`).
- **▶ Prontos agora (deps satisfeitas):** **D2** (runner `#test` no interpretador — destrava D3/D4, torna os 646 asserts executáveis) · **A4** (regra do main pelo artefato) · **A5!**+**A5b** (projeto-só + mirror dos mains — correções) · **B1!** (aderência do `teko::assert`) · **E1** (posição na pipeline) · **C1.0** (legislar `extern` — você).
- **Em seguida:** A4 → A5! → A5b (Eixo A/entrada); D2 → D3 → D4 (testes no interpretador); E1 → E2 → {E3,E4} (símbolos/diagnósticos); C1.0 → C1.1 → C2* (FFI/host).
...
- **Eixos A–D (projeto, testes, independência, interpretador)** → **Phase 7** (host independence) ✅
